...
action :create do
  ...
  cr = cron "curator-#{cur_instance}" do
    command "#{cur_bin_dir}/curator --host #{server_ip} delete indices --time-unit days --timestring '%Y.%m.%d' --older-than #{cur_days_to_keep} --prefix #{cur_index_prefix} &> #{cur_log_file}"
    user    cur_user
    minute  cur_minute
    hour    cur_hour
    action  [:create]
...
action :delete do
  ...
  cr = cron "curator-#{cur_instance}" do
    command "#{cur_bin_dir}/curator --host #{::Logstash.service_ip(node, cur_instance, 'elasticsearch')} delete indices --time-unit days --timestring '%Y.%m.%d' --older-than #{cur_days_to_keep} --prefix #{cur_index_prefix} 2>&1 > #{cur_log_file}"
    user    cur_user
    minute  cur_minute
    hour    cur_hour
    action  [:delete]
